...
// **************************************************************************************
	...
	//pBeholder->textures[1] = "Beholder_Base_color.bmp";
	//pBeholder->textures[2] = "Beholder_Base_color.bmp";
	//pBeholder->textures[3] = "Beholder_Base_color.bmp";
	...
	*pBeholder3 = *pBeholder;
	...
	cMeshObject* pBeholderA2 = new cMeshObject();
	*pBeholderA2 = *pBeholder;
	pBeholderA2->id = 9831;
	pBeholderA2->friendlyName = "pBeholderA2";
	pBeholderA2->position.x += 50.f;
	pBeholderA2->SetUniformScale(5.f);
	...
	//bool increase = true;
	int increase = 1;
	player->meshName = "Cube";
	player->friendlyName = "Player";
	player->bUse_RGBA_colour = true;      
	player->position = glm::vec3(0);
	player->setRotationFromEuler(glm::vec3(0));
	player->isWireframe = false;
	player->SetUniformScale(5.f);
	Sphere * playerSphere = new Sphere(Point(0.0f, 0.0f, 0.0f), 5);
	player->physics_object = g_physics_system.CreatePhysicsObject(player->position, playerSphere, player->friendlyName);
	player->hasPhysicsObject = true;
	player->bDoNotLight = false;
	player->RGBA_colour = glm::vec4(1, 0, 1, 1);
	g_pMeshObjects.push_back(player);
	pVAOManager->Load();
	// Actors
	Sphere* sphere = new Sphere(Point(0.0f, 0.0f, 0.0f), 5);
	Sphere* sphere2 = new Sphere(Point(0.0f, 0.0f, 0.0f), 5);
	Sphere* sphere3 = new Sphere(Point(0.0f, 0.0f, 0.0f), 5);
	pBeholder->physics_object = g_physics_system.CreatePhysicsObject(pBeholder->position, sphere, pBeholder->friendlyName);
	pBeholder->hasPhysicsObject = true;
	pBeholder2->physics_object = g_physics_system.CreatePhysicsObject(pBeholder2->position, sphere2, pBeholder2->friendlyName);
	pBeholder2->hasPhysicsObject = true;
	pBeholder3->physics_object = g_physics_system.CreatePhysicsObject(pBeholder3->position, sphere3, pBeholder3->friendlyName);
	pBeholder3->hasPhysicsObject = true;
	AIBrain ai_system(player);
	Actor enemy1(pBeholder, AItype::A);
	Actor enemy2(pBeholder2, AItype::B, 1.3f);
	Actor enemy3(pBeholder3, AItype::C, 100, 90);
	ai_system.actors_.push_back(enemy1);
	ai_system.actors_.push_back(enemy2);
	ai_system.actors_.push_back(enemy3);
	pBeholder2->bUse_RGBA_colour = true;
	pBeholder2->RGBA_colour = glm::vec4(1, 0, 0,1);
	pBeholder3->bUse_RGBA_colour = true;
	pBeholder3->RGBA_colour = glm::vec4(1, 1, 0, 1);
	pBeholder->physics_object->position.y = 15.f;
	pBeholder2->physics_object->position.y = 15.f;
	pBeholder3->physics_object->position.y = 15.f;

	// 2ND SET


	cMeshObject* pBeholderB2 = new cMeshObject();
	pBeholderB2->meshName = "Beholder";
	pBeholderB2->friendlyName = "BeholderB2";
	pBeholderB2->bUse_RGBA_colour = true;      // Use file colours    pTerrain->RGBA_colour = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f);
	pBeholderB2->RGBA_colour = glm::vec4(1, 0, 0, 1);
	pBeholderB2->position = glm::vec3(-300.f, 15.f, 200.f);
	pBeholderB2->setRotationFromEuler(glm::vec3(0));
	pBeholderB2->isWireframe = false;
	pBeholderB2->SetUniformScale(5.f);
	pBeholderB2->textures[0] = "Beholder_Base_color.bmp";
	pBeholderB2->textureRatios[0] = 1.0f;

	cMeshObject* pBeholderC2 = new cMeshObject();
	pBeholderC2->meshName = "Beholder";
	pBeholderC2->friendlyName = "BeholderC2";
	pBeholderC2->bUse_RGBA_colour = true;
	pBeholderC2->RGBA_colour = glm::vec4(1, 1, 0, 1);
	pBeholderC2->position = glm::vec3(300.f, 15.f, 200.f);
	pBeholderC2->setRotationFromEuler(glm::vec3(0));
	pBeholderC2->isWireframe = false;
	pBeholderC2->SetUniformScale(5.f);
	pBeholderC2->textures[0] = "Beholder_Base_color.bmp";
	pBeholderC2->textureRatios[0] = 1.0f;
	g_pMeshObjects.push_back(pBeholderA2);
	g_pMeshObjects.push_back(pBeholderB2);
	g_pMeshObjects.push_back(pBeholderC2);
	// Spheres
	Sphere* sphereA2 = new Sphere(Point(0.0f, 0.0f, 0.0f), 5);
	Sphere* sphereB2 = new Sphere(Point(0.0f, 0.0f, 0.0f), 5);
	Sphere* sphereC2 = new Sphere(Point(0.0f, 0.0f, 0.0f), 5);
	// Physic Objects
	pBeholderA2->physics_object = g_physics_system.CreatePhysicsObject(pBeholderA2->position, sphereA2, pBeholderA2->friendlyName);
	pBeholderB2->physics_object = g_physics_system.CreatePhysicsObject(pBeholderB2->position, sphereB2, pBeholderB2->friendlyName);
	pBeholderC2->physics_object = g_physics_system.CreatePhysicsObject(pBeholderC2->position, sphereC2, pBeholderC2->friendlyName);
	pBeholderA2->hasPhysicsObject = true;
	pBeholderB2->hasPhysicsObject = true;
	pBeholderC2->hasPhysicsObject = true;
	pBeholderA2->physics_object->position.y = 15.f;
	pBeholderB2->physics_object->position.y = 15.f;
	pBeholderC2->physics_object->position.y = 15.f;
	// Position 
	ai_system.actors_.push_back(Actor(pBeholderA2, AItype::A, 1.2f));
	ai_system.actors_.push_back(Actor(pBeholderB2, AItype::B, 1.4f));
	ai_system.actors_.push_back(Actor(pBeholderC2, AItype::C));
	while (!glfwWindowShouldClose(window))
	{
		player->position = glm::vec3(player->physics_object->position.x, g_cameraEye.y -300.f, player->physics_object->position.z);
		g_cameraEye.x = player->position.x -100.f;
		g_cameraEye.z = player->position.z -100.f;
		pBeholder->position = pBeholder->physics_object->position;
		pBeholder2->position = pBeholder2->physics_object->position;
		pBeholder3->position = pBeholder3->physics_object->position;
		pBeholderA2->position = pBeholderA2->physics_object->position;
		pBeholderB2->position = pBeholderB2->physics_object->position;
		pBeholderC2->position = pBeholderC2->physics_object->position;
	...
	//	command_group->Update(0.1);

		//if (glm::length(pBeholder3->position - glm::vec3(60, 40, 150)) <= 5.f && firstTime)
		//{
		////	pBrain->RunScriptImmediately("Expand( " + std::to_string(pBeholder3->id) + ", 500)");
		//	/*pBrain->RunScriptImmediately("Invisible( " + std::to_string(pBeholder3->id) + " )");
		//	firstTime = false;*/
		//}
		...
		if (duration > 0.1 && !menuMode)
		{
			...
			player->physics_object->Integrate(0.1f);
			deltaTime = std::clock();
		}
